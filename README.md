...
`content/research/updates/<target>/`; earlier evidence retains its original version.
...
- **Helper:** [`scripts/content_version.py`](scripts/content_version.py) is the shared parser used by
  ...
  python scripts\content_version.py tag         # content-v<edition>
...
`content-v<version>` with the matching single-file PDF attached. This is automated by
...
5. The gated deploy and release workflows publish the site and `content-v<edition>` with
